...
// Shared chart-building logic used by the local data-to-chart renderer.
// The browser dashboard keeps an identical offline copy in chart-local.js.
const PALETTE = [
  '#6366f1', '#ec4899', '#22c55e', '#f59e0b', '#06b6d4',
  '#a855f7', '#ef4444', '#14b8a6', '#eab308', '#8b5cf6',
];

// Group rows by the X-axis value and sum each numeric Y column.
function aggregate(rows, xAxis, yAxis) {
  const map = new Map();
  for (const r of rows) {
...
// Turn raw request payload into a ready-to-draw Chart.js config.
function buildConfig({ type, xAxis, yAxis, rows }) {
  const agg = aggregate(rows, xAxis, yAxis);
...
module.exports = { PALETTE, aggregate, buildConfig };
